...
const char* logLevelColor(Logger::LogLevel log_level) {
  switch (log_level) {
...
{
  std::cerr << logLevelColor(logLevel) << qPrintable(formattedString(timeStamp, logLevel, file, line, function, category, message));
}
