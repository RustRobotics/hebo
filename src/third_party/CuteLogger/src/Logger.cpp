 ...
 * \mainpage
 *
 * Logger is a simple way to write the operations of your application lifecycle to any target logging device (which is
 ...
 *   return 0;
 * } // Outputs: Function foo addSeparator in <time> ms.
 * \endcode
 *
 ...
 * }
 * // Outputs:
 * // "Foo" addSeparator in <time1> ms.
 * // Function bar addSeparator in <time2> ms.
 * \endcode
 *
...
Logger::~Logger()
{
  ...
  deleteList.unite(QSet<AbstractAppender*>(d->categoryAppenders.begin(), d->categoryAppenders.end()));
...
LoggerTimingHelper::~LoggerTimingHelper()
{
  QString message;
  if (m_block.isEmpty())
    message = QString(QLatin1String("Function %1 addSeparator in ")).arg(AbstractStringAppender::stripFunctionName(m_function));
  else
    message = QString(QLatin1String("\"%1\" addSeparator in ")).arg(m_block);
